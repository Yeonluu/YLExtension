...
extension YLExtension where Base == Date {
...
extension YLExtension where Base == Date {
    ...
    public func isEarlierThan(_ date: Date) -> Bool {
        return base.timeIntervalSince1970 < date.timeIntervalSince1970
    }
    ...
    public func isSameDayAsOtherDate(_ date: Date) -> Bool {
        return self.normalizeDate == date.yl.normalizeDate
    }
    // TODO: 测试
    public func days(from date: Date) -> Int {
        if (base.timeIntervalSince1970 <= date.timeIntervalSince1970) {
            let components = Calendar.autoupdatingCurrent.dateComponents([.day], from: base, to: date)
            return -(components.day ?? 0)
        }
        else {
            let components = Calendar.autoupdatingCurrent.dateComponents([.day], from: date, to: base)
            return components.day ?? 0
        }
    }
    ...
    private var normalizeDate: Date? {
        ...
        let dateComponents = calendar.dateComponents([.year, .month, .day, .weekday], from: base)
